...
export default async function EditAssignmentPage({ params }: PageProps) {
  ...
  if (!assignment) {
    return <DashboardLayout><div className="space-y-6"><h1 className="text-3xl font-bold">Assignment Not Found</h1></div></DashboardLayout>;
  }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <TeamAssignmentForm assignmentId={id} initialData={{ title: assignment.title, description: assignment.description, assignmentType: assignment.assignmentType, sourceModule: assignment.sourceModule, referenceId: assignment.referenceId, assignedToUserId: assignment.assignedToUserId, assignedByUserId: assignment.assignedByUserId, status: assignment.status, priority: assignment.priority, dueDate: assignment.dueDate ? new Date(assignment.dueDate).toISOString().slice(0, 10) : "", notes: assignment.notes }} members={members} />
          </CardContent>
        </Card>
      </div>
    </DashboardLayout>
