...
export default async function TeamPerformancePage({ searchParams }: PageProps) {
  ...
  const perf = await getPerformance(params);
  ...
  const avgCompletion =
    items.length > 0
      ? Math.round(
          items.reduce((s: number, m: { completionRate?: number }) => s + (m.completionRate ?? 0), 0) /
            items.length
        )
      : 0;
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            {items.length === 0 ? (
              <p className="text-muted-foreground">No performance data. Add team members and assignments.</p>
            ) : (
            ...
            <CardContent>
              {topPerformers.length === 0 ? (
                ...
                <ul className="space-y-2">
                  {topPerformers.map((m: { memberName: string; completionRate: number }) => (
          ...
          <Card>
            ...
            <CardContent>
              {lowActivity.length === 0 ? (
                ...
                <ul className="space-y-2">
                  {lowActivity.map((m: { memberName: string; completed: number }) => (
                    <li key={m.memberName} className="flex justify-between">
                      <span>{m.memberName}</span>
                      <span className="text-muted-foreground">{m.completed} completed</span>
                    </li>
                  ))}
                </ul>
              )}
