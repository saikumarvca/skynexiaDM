...
async function getMembers(params: {
  ...
  const emails = [
    ...new Set(serialized.map((m) => String(m.email).trim().toLowerCase())),
  ];
  const usersWithPassword =
    emails.length > 0
      ? await User.find({
          ...
          .lean()
      : [];
  const loginByEmail = new Set(
    usersWithPassword.map((u) => String(u.email).toLowerCase()),
  );
...
async function getRoles() {
  await dbConnect();
  const items = await TeamRole.find({ isDeleted: { $ne: true } })
    .sort({ roleName: 1 })
    .limit(100)
    .lean();
...
export default async function TeamMembersPage({ searchParams }: PageProps) {
  ...
  const assignedCount = (m: { assignedClientIds?: unknown[] }) =>
                  ...
                  <TableBody>
                    {members.map((m) => (
                      <TableRow key={m._id}>
                        ...
                        <TableCell>
                          <span
                            className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${m.status === "Active" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}
                          >
                            {m.status}
                          </span>
                        </TableCell>
                ...
                {membersData.totalPages > 1 && (
                  <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
                    ...
                    <div className="flex gap-2">
                      {membersData.page > 1 && (
                        <Link
                          href={`/team/members?${new URLSearchParams({ ...params, page: String(membersData.page - 1) })}`}
                        >
