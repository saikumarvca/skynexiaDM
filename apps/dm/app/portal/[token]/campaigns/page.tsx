...
export default async function PortalCampaignsPage({ params }: CampaignsPortalPageProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-gray-500">Your active and completed campaigns.</p>
      </div>
      ...
      {campaigns.length === 0 ? (
        ...
        <div className="space-y-4">
          {campaigns.map((camp) => {
            ...
            const m = c.metrics ?? {};
            return (
              <div key={String(c._id)} className="rounded-lg border bg-white p-5 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-semibold text-gray-900">{c.campaignName ?? 'Campaign'}</h3>
                    {c.platform && (
                      <p className="mt-0.5 text-sm text-gray-500">Platform: {c.platform}</p>
                    )}
                    {c.objective && (
                      <p className="text-sm text-gray-500">Objective: {c.objective}</p>
                    )}
                  </div>
                  <span
