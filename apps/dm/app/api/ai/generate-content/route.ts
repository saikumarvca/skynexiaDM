...
function buildPrompt(type: ContentType, platform: string, topic: string, tone?: string): string {
...
export async function POST(req: NextRequest) {
  ...
  const validTypes: ContentType[] = ["CAPTION", "HASHTAGS", "AD_COPY", "CTA", "HOOK"]
  if (!type || !validTypes.includes(type)) {
    return NextResponse.json({ error: "type must be one of: CAPTION, HASHTAGS, AD_COPY, CTA, HOOK" }, { status: 400 })
  }
  ...
  if (saveToBank && clientId) {
    try {
      await dbConnect()
      const titleParts = [type.toLowerCase().replace("_", " "), platform, topic.slice(0, 40)]
      ...
      console.error("[ai/generate-content] Failed to save to content bank:", err)
    }
  }
