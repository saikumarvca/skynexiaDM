...
export async function PATCH(
  ...
  try {
    ...
    if (body.email !== undefined) {
      set.email = typeof body.email === "string" && body.email.trim() ? body.email.trim() : undefined
    }
    if (body.phone !== undefined) {
      set.phone = typeof body.phone === "string" && body.phone.trim() ? body.phone.trim() : undefined
    }
    if (body.source !== undefined) {
      set.source = typeof body.source === "string" && body.source.trim() ? body.source.trim() : undefined
    }
    ...
    if (body.campaignId !== undefined) {
      ...
      } else if (typeof body.campaignId === "string" && mongoose.Types.ObjectId.isValid(body.campaignId)) {
    ...
    const updated = await Lead.findByIdAndUpdate(leadId, { $set: set }, { new: true })
      .populate("clientId", "name businessName")
      .populate("campaignId", "campaignName")
      .lean()
