...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [reviews, campaigns, leads, tasks, contentItems, keywords, scheduledPosts, reviewRequests] =
      await Promise.all([
        ...
        Promise.resolve(ReviewRequest.find({ clientId: { $in: clientIds } }).lean()).catch(() => []),
      ]);

    // Group data by client
    const groupById = <T extends { clientId: unknown }>(arr: T[], key: string) => {
    ...
    const requestsByClient = groupById(reviewRequests as unknown as Array<{ clientId: unknown }>, 'clientId');
