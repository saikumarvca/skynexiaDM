...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!posted) {
      return NextResponse.json({ error: 'Posted review not found' }, { status: 404 });
    }

    const allocId =
      typeof posted.allocationId === 'object' && posted.allocationId != null && '_id' in posted.allocationId
        ? String((posted.allocationId as { _id: unknown })._id)
        : String(posted.allocationId);
    const draftId =
      typeof posted.draftId === 'object' && posted.draftId != null && '_id' in posted.draftId
