...
export function CampaignsListWithSheet({ campaigns, clients }: CampaignsListWithSheetProps) {
  ...
  const allChecked = campaigns.length > 0 && checkedIds.size === campaigns.length
  ...
  async function bulkUpdateStatus() {
    ...
    try {
      ...
      toast.success(`Updated ${data.updated} campaign${data.updated !== 1 ? "s" : ""}`)
      setCheckedIds(new Set())
      setBulkStatus("")
      router.refresh()
    } catch {
      ...
      {checkedIds.size > 0 && (
        <div className="mb-3 flex flex-wrap items-center gap-3 rounded-md border bg-muted/50 px-4 py-2.5">
          ...
          <div className="flex items-center gap-2">
            <select
              value={bulkStatus}
              onChange={(e) => setBulkStatus(e.target.value as CampaignStatus | "")}
          ...
          {selected && (
            <>
              <SheetHeader className="shrink-0 space-y-1 border-b px-6 py-4 pr-14 text-left">
                ...
                <SheetDescription>
                  View and edit this campaign. To remove from active work, set status to ARCHIVED and
                  save.
                </SheetDescription>
              </SheetHeader>
