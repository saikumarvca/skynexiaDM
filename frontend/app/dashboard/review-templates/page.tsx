...
export default async function ReviewTemplatesPage() {
  ...
  async function createTemplate(formData: FormData) {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        (err as { error?: string }).error || "Failed to create template",
      );
    }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Prefill review fields when writers pick a template on the new-review
            form.
          </p>
        </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-muted/40 text-left">
                      ...
                      <th className="w-28 p-3 font-medium text-right">
                        Actions
                      </th>
                    </tr>
                  </thead>
                        ...
                        <td className="p-3 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button variant="ghost" size="sm" asChild>
                              <Link
                                href={`/dashboard/review-templates/${t._id}/edit`}
                              >
                                Edit
                              </Link>
                            </Button>
