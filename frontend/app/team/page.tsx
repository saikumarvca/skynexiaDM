...
export default async function TeamOverviewPage() {
  ...
  for (const a of assignments) {
    assignmentsByMember[a.assignedToUserId] =
      (assignmentsByMember[a.assignedToUserId] || 0) + 1;
  }

  const overloaded = members
    .filter(
      (m) =>
        getWorkloadStatus(assignmentsByMember[m._id.toString()] || 0) ===
        "Overloaded",
    )
    .map((m) => ({ memberName: m.name, memberId: m._id.toString() }));
  ...
  const recentlyActive = recentActivity
    .filter((a) => {
    ...
    .slice(0, 5)
    .map((a) => ({ memberName: a.userName, userId: a.userId }));
