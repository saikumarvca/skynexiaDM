...
export function AuditLogClient({ reviewLogs, teamLogs }: AuditLogClientProps) {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeTab === "review")
      items = items.filter((i) => i.type === "review");
    ...
    if (dateFrom) {
      ...
      items = items.filter(
        (i) =>
          isAfter(i.timestamp, from) ||
          i.timestamp.getTime() === from.getTime(),
      );
    }
    if (dateTo) {
      ...
      items = items.filter(
        (i) =>
          isBefore(i.timestamp, to) || i.timestamp.getTime() === to.getTime(),
      );
    }
